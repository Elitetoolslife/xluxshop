    ...
    <tbody>
        ...
        while ($row = mysqli_fetch_assoc($q)) {
            ...
            while ($rpw = mysqli_fetch_assoc($qer)) {
                ...
                echo "
                <tr>
                    <td id='huntingtonbanks_country'><i class='flag-icon flag-icon-$countrycode'></i>&nbsp;" . htmlspecialchars($row['country']) . "</td>
                    ...
                    <td>" . $row['date'] . "</td>
                    <td><span id='huntingtonbanks{$row['id']}' title='buy' type='huntingtonbanks'><a onclick='buythistool({$row['id']})' class='btn btn-primary btn-xs'><font color=white>Buy</font></a></span></td>
                </tr>
                ";
            }
        }
...
$(document).ready(function() {
    ...
    $.fn.dataTable.ext.search.push(
        function(settings, data, dataIndex) {
            ...
            var fk1 = $.trim($("#huntingtonbanks_country").val().toLowerCase());
            var fk2 = $.trim($("#huntingtonbanks_sitename").val().toLowerCase());
            var fk3 = $.trim($("#huntingtonbanks_seller").val().toLowerCase());
        ...
        if (result.isConfirmed) {
            Swal.fire({
                ...
                didOpen: () => {
                    Swal.showLoading()
                    $.ajax({
                        url: `buy/huntingtonbanks.php?id=${id}`,
                        method: 'GET',
                        dataType: 'json',
                        success: function(data) {
                            ...
                            if (data.status === 'success') {
                                Swal.fire('Success!', 'Order placed successfully!', 'success').then(() => {
                                    $("#huntingtonbanks" + id).html('<button onclick="openitem(' + data.order_id + ')" class="btn btn-primary btn-xs"> Order #' + data.order_id + '</button>');
                                });
                            } else {

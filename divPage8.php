    ...
    <div class="tab-pane active in" id="filter">
        <div class="filter-container">
            <div class="filter-row">
                <label for="hungtingtonbanks_country">Country:</label>
                <select class="filterselect form-control input-sm" name="hungtingtonbanks_country" id="hungtingtonbanks_country">
                    <option value="">ALL</option>
                    <?php
                    $query = mysqli_query($dbcon, "SELECT DISTINCT `country` FROM `hungtingtonbanks` WHERE `sold` = '0' ORDER BY country ASC");
            ...
            <div class="filter-row">
                <label for="hungtingtonbanks_sitename">Site Name:</label>
                <input class="filterinput form-control input-sm" name="hungtingtonbanks_sitename" id="hungtingtonbanks_sitename" size="3">
            </div>
            <div class="filter-row">
                <label for="hungtingtonbanks_seller">Seller:</label>
                <select class="filterselect form-control input-sm" name="hungtingtonbanks_seller" id="hungtingtonbanks_seller">
                    <option value="">ALL</option>
                    <?php
                    $query = mysqli_query($dbcon, "SELECT DISTINCT `resseller` FROM `hungtingtonbanks` WHERE `sold` = '0' ORDER BY resseller ASC");
                    while ($row = mysqli_fetch_assoc($query)) {
...
<table id="table" class="display">
    ...
    <tbody>
        <?php
        include("cr.php");
        $q = mysqli_query($dbcon, "SELECT * FROM hungtingtonbanks WHERE sold='0' ORDER BY RAND()")or die(mysqli_error());
        while ($row = mysqli_fetch_assoc($q)) {
            ...
            while ($rpw = mysqli_fetch_assoc($qer)) {
                ...
                echo "
                <tr>
                    <td id='hungtingtonbanks_country'><i class='flag-icon flag-icon-$countrycode'></i>&nbsp;" . htmlspecialchars($row['country']) . "</td>
                    ...
                    <td>" . $row['date'] . "</td>
                    <td><span id='hungtingtonbanks{$row['id']}' title='buy' type='hungtingtonbanks'><a onclick='buythistool({$row['id']})' class='btn btn-primary btn-xs'><font color=white>Buy</font></a></span></td>
                </tr>
                ";
            }
        }
...
$(document).ready(function() {
    ...
    $.fn.dataTable.ext.search.push(
        function(settings, data, dataIndex) {
            ...
            var fk1 = $.trim($("#hungtingtonbanks_country").val().toLowerCase());
            var fk2 = $.trim($("#hungtingtonbanks_sitename").val().toLowerCase());
            var fk3 = $.trim($("#hungtingtonbanks_seller").val().toLowerCase());
        ...
        if (result.isConfirmed) {
            Swal.fire({
                ...
                didOpen: () => {
                    Swal.showLoading()
                    $.ajax({
                        url: `buy/hungtingtonbanks.php?id=${id}`,
                        method: 'GET',
                        dataType: 'json',
                        success: function(data) {
                            ...
                            if (data.status === 'success') {
                                Swal.fire('Success!', 'Order placed successfully!', 'success').then(() => {
                                    $("#hungtingtonbanks" + id).html('<button onclick="openitem(' + data.order_id + ')" class="btn btn-primary btn-xs"> Order #' + data.order_id + '</button>');
                                });
                            } else {

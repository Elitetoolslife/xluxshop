        ...
        <div class="row">
            <form class="login" method="post" action="signupform.html">
                ...
                @if(isset($_GET['error']) && !empty($_GET['error']))
                    ...
                    @endphp
                    @if ($error == "userexist")
                        <div class='alert alert-dismissible alert-info'><button type='button' class='close' data-dismiss='alert'>×</button><p>Username already exists!</p></div>
                    @elseif ($error == "emailexist")
                        <div class='alert alert-dismissible alert-info'><button type='button' class='close' data-dismiss='alert'>×</button><p>Email already exists!</p></div>
                    @elseif ($error == "passnotmatch")
                        <div class='alert alert-dismissible alert-info'><button type='button' class='close' data-dismiss='alert'>×</button><p>Passwords do not match!</p></div>
                    @elseif ($error == "passlength")
                        <div class='alert alert-dismissible alert-info'><button type='button' class='close' data-dismiss='alert'>×</button><p>Password must be more than 6 and less than 20 characters!</p></div>
                    @endif
                @endif
